...
if '<select class="select" id="fSituacao"><option value="">Todas</option><option>PENDENTE</option>' not in html:
    html = html.replace(
...
if '<select class="select" id="mSituacao"><option>PENDENTE</option>' not in html:
    html = html.replace(
...
js = js.replace('const situacao=up(r.situacao||"PORTA")', 'const situacao=up(r.situacao||"PENDENTE")')
js = js.replace('const old=modal.value||"PORTA"', 'const old=modal.value||"PENDENTE"')
js = js.replace('modal.value=STATUS_OPTIONS.includes(old)?old:"PORTA"', 'modal.value=STATUS_OPTIONS.includes(old)?old:"PENDENTE"')
...
js = js.replace('const old=select.dataset.old||"PORTA",next=up(select.value)', 'const old=select.dataset.old||"PENDENTE",next=up(select.value)')
...
print('Embarques ajustado: PENDENTE incluído, default PENDENTE e campos opcionais no front-end.')
