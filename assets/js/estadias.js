...
function moneyInput(v){return num(v).toLocaleString("pt-BR",{style:"currency",currency:"BRL",minimumFractionDigits:2,maximumFractionDigits:2})}
function dateTimeBR(v){if(!v)return "-";if(typeof v==="number"){const excelEpoch=new Date(Date.UTC(1899,11,30));const d=new Date(excelEpoch.getTime()+v*86400000);return d.toLocaleString("pt-BR",{dateStyle:"short",timeStyle:"medium"})}const raw=txt(v);const parsed=Date.parse(raw);if(!Number.isNaN(parsed))return new Date(parsed).toLocaleString("pt-BR",{dateStyle:"short",timeStyle:"medium"});return raw}
function dateOnlyBR(v){if(!v)return "-";const d=new Date(v);return Number.isNaN(d.getTime())?txt(v):d.toLocaleDateString("pt-BR")}
function toDateInput(v){if(!v)return "";const d=new Date(v);if(Number.isNaN(d.getTime()))return "";return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}-${String(d.getDate()).padStart(2,"0")}`}
function toDateTimeInput(v){if(!v)return "";const d=new Date(v);if(Number.isNaN(d.getTime()))return "";return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}-${String(d.getDate()).padStart(2,"0")}T${String(d.getHours()).padStart(2,"0")}:${String(d.getMinutes()).padStart(2,"0")}:${String(d.getSeconds()).padStart(2,"0")}`}
function parseDateValue(v){if(!v)return null;if(v instanceof Date)return Number.isNaN(v.getTime())?null:v;if(typeof v==="number"){const epoch=new Date(Date.UTC(1899,11,30));const d=new Date(epoch.getTime()+v*86400000);return Number.isNaN(d.getTime())?null:d}const raw=txt(v);const br=raw.match(/^(\d{1,2})\/(\d{1,2})\/(\d{4})(?:[,\s]+(\d{1,2}):(\d{2})(?::(\d{2}))?)?$/);if(br){const d=new Date(+br[3],+br[2]-1,+br[1],+(br[4]||0),+(br[5]||0),+(br[6]||0));return Number.isNaN(d.getTime())?null:d}const d=new Date(raw);return Number.isNaN(d.getTime())?null:d}
function hoursBetween(start,end){const a=parseDateValue(start),b=parseDateValue(end);if(!a||!b)return 0;return Math.max(0,(b-a)/3600000)}
...
function authContext(){if(typeof getAuthContext==="function")return getAuthContext();return {usuario:localStorage.getItem("nf_auth_user")||"USUARIO",nome:localStorage.getItem("nf_auth_name")||localStorage.getItem("nf_auth_user")||"USUARIO",perfil:localStorage.getItem("nf_auth_profile")||"PERFIL",estado:localStorage.getItem("nf_auth_state")||""}}
...
async function updateValorPago(id,valorPago,inputEl=null){if(!ensureWrite())return false;const item=dados.find(r=>r.id===id);if(!item){alert("Registro não encontrado.");return false}const novoValor=Math.max(0,roundMoney(valorPago));const valorAnterior=Math.max(0,roundMoney(item.valorPago));if(novoValor===valorAnterior){if(inputEl)inputEl.value=moneyInput(novoValor);return true}if(inputEl){inputEl.disabled=true;inputEl.classList.add("saving")}try{const res=await portalCall("update",{...item,valorPago:novoValor,resource:"registros",...auditPayload()});if(!res||res.ok===false)throw new Error(res?.error||"Falha ao atualizar o valor pago.");item.valorPago=novoValor;aplicarFiltros();return true}catch(error){console.error("[ESTADIAS] Erro ao atualizar valor pago:",error);item.valorPago=valorAnterior;if(inputEl)inputEl.value=moneyInput(valorAnterior);alert(`Não foi possível atualizar o Valor Pago.\n\n${error.message}`);return false}finally{if(inputEl){inputEl.disabled=false;inputEl.classList.remove("saving")}}}
...
function renderStatusCell(r){const statusAtual=normalizeStatus(r.status);if(!canWrite())return `<span title="Status atual da solicitação" style="display:inline-flex;align-items:center;justify-content:center;min-width:132px;min-height:28px;padding:0 10px;border-radius:999px;font-size:8px;font-weight:1000;text-transform:uppercase;white-space:nowrap;${readonlyStatusStyle(statusAtual)}">${esc(statusAtual)}</span>`;return `<select class="statusSelect ${statusClass(statusAtual)}" data-id="${esc(r.id)}" data-old-status="${esc(statusAtual)}" ${statusEmAtualizacao.has(r.id)?"disabled":""}>${statusOptionsHtml(statusAtual)}</select>`}
function renderValorPagoCell(r){if(canWrite()){const raw=Math.max(0,num(r.valorPago));const display=moneyInput(raw);return `<input class="paidValueInput" type="text" inputmode="decimal" data-id="${esc(r.id)}" value="${esc(display)}" title="Clique para editar o valor pago" aria-label="Valor pago" autocomplete="off" style="width:112px;height:30px;border:1px solid transparent;border-radius:8px;background:transparent;color:#086f47;padding:0;text-align:left;font-size:9px;font-weight:900;outline:none;cursor:text;box-shadow:none">`}return `<span class="paidValueReadOnly" style="color:#086f47;font-weight:900">${moneyInput(r.valorPago||0)}</span>`}
function renderActionsCell(r){const actions=[];if(canWrite())actions.push(`<button class="btn ghost btnEdit" data-id="${esc(r.id)}" data-estadias-write style="min-height:28px;padding:0 9px">✏ Editar</button>`);if(r.anexoUrl)actions.push(`<a class="btn ghost" href="${esc(r.anexoUrl)}" target="_blank" rel="noopener" style="min-height:28px;padding:0 9px">📎</a>`);if(canDelete())actions.push(`<button class="btn red btnDelete" data-id="${esc(r.id)}" data-estadias-delete style="min-height:28px;padding:0 9px">🗑</button>`);return actions.length?`<div style="display:flex;gap:5px;align-items:center">${actions.join("")}</div>`:'<span style="color:#8795aa">Consulta</span>'}
