/* fretes2.js | NOVA FROTA | Fretes MT | Cadência operacional */
(function () {
  "use strict";
  ...
  const COLS = [
    ...
    { key: "cadencia", label: "Cadência", isInlineEditable: true },
    { key: "porta", label: "Porta", isInlineEditable: true },
    { key: "transito", label: "Trânsito", isInlineEditable: true },
    { key: "carregado", label: "Carregado", isInlineEditable: true },
    { key: "aderencia", label: "Aderência %", isAderencia: true },
  ...
  const MODAL = {
    ...
    cadencia: () => document.getElementById("mCadencia"),
    ...
    carregado: () => document.getElementById("mCarregado"),
  ...
  async function waitForPortalReady() {
    ...
    if (typeof canAccessFeature === "function" && !canAccessFeature("fretes2")) {
...
function formatDateTimeBR(value) {
  ...
  function calculateAdherence(row) {
    const cadencia = parsePtNumber(row?.cadencia) || 0;
    if (cadencia <= 0) return 0;

    const realizado =
      (parsePtNumber(row?.porta) || 0) +
      (parsePtNumber(row?.transito) || 0) +
      (parsePtNumber(row?.carregado) || 0);

    return Math.round(((realizado / cadencia) * 100 + Number.EPSILON) * 10) / 10;
  }

  function formatAdherence(value) {
    const n = parsePtNumber(value);
    const v = Number.isFinite(n) ? n : 0;
    const fixed = Number.isInteger(v) ? v.toFixed(0) : v.toFixed(1).replace(".", ",");
    return `${fixed}%`;
  }

  function buildAdherenceCell(row) {
    const td = document.createElement("td");
    td.className = "num";

    const value = Number.isFinite(parsePtNumber(row?.aderencia))
      ? parsePtNumber(row.aderencia)
      : calculateAdherence(row);

    const span = document.createElement("span");
    span.className = "nfAdherenceBadge";
    span.textContent = formatAdherence(value);
    span.title = `Cadência: ${safeText(row?.cadencia || 0)} | Porta: ${safeText(row?.porta || 0)} | Trânsito: ${safeText(row?.transito || 0)} | Carregado: ${safeText(row?.carregado || 0)}`;
    ...
    if (value < 70) {
      ...
      span.style.borderColor = "#86EFAC";
    } else {
      span.style.background = "#DBEAFE";
      span.style.color = "#1D4ED8";
      span.style.borderColor = "#93C5FD";
    }
    ...
    async function commit() {
      ...
      try {
        ...
        const res = await apiGet({
          action: "fretes2_update",
          id: safeText(row.id),
          [key]: newValue
        });

        const serverRow = res?.data && typeof res.data === "object" ? res.data : {};
        const idx = STATE.rows.findIndex((r) => safeText(r.id) === safeText(row.id));

        if (idx >= 0) {
          Object.assign(STATE.rows[idx], serverRow, { [key]: newValue });
          STATE.rows[idx].ultimaAlteracao = serverRow.ultimaAlteracao || ultimaAlteracao;
          if (serverRow.aderencia === undefined) {
            STATE.rows[idx].aderencia = calculateAdherence(STATE.rows[idx]);
          }
        }

        Object.assign(row, serverRow, { [key]: newValue });
        row.ultimaAlteracao = serverRow.ultimaAlteracao || ultimaAlteracao;
        if (serverRow.aderencia === undefined) row.aderencia = calculateAdherence(row);
  ...
  function render(rowsRaw) {
    ...
    rows.forEach((row) => {
      ...
      COLS.forEach((col) => {
        ...
        if (col.isAderencia) {
          tr.appendChild(buildAdherenceCell(row));
          return;
        }
        ...
        if (["cadencia", "valorEmpresa", "valorMotorista", "km", "pedagioEixo", "pedidoSat", "porta", "transito", "carregado", "aderencia"].includes(col.key)) {
          td.className = "num";
        }
  ...
  async function canvasFromPreview(row) {
    ...
    try {
      ...
      console.error("[fretes2] erro ao gerar canvas:", e);
  ...
  async function downloadDivulgacaoJPG(row) {
    ...
    try {
      ...
      console.error("[fretes2] erro ao baixar JPG:", e);
  ...
  async function copyPreviewImage(row) {
    ...
    try {
      ...
      console.warn("[fretes2] copiar imagem falhou:", e);
  ...
  function renderStats(rows) {
    ...
    set("nfStatVolume", `${sum("cadencia").toLocaleString("pt-BR", { maximumFractionDigits: 0 })} un`);
...
<div class="page">
  ...
  function fillModalFromRow(row) {
    ...
    if (MODAL.cadencia()) MODAL.cadencia().value = safeText(row.cadencia ?? row.volume);
    ...
    if (MODAL.carregado()) MODAL.carregado().value = safeText(row.carregado);
    ...
    try {
      ...
      if (savedId) {
        const nextRow = {
          ...
          ...serverRow,
          id: savedId,
          cadencia: serverRow.cadencia ?? payload.cadencia ?? 0,
          carregado: serverRow.carregado ?? payload.carregado ?? 0,
        ...
        nextRow.aderencia = Number.isFinite(parsePtNumber(serverRow.aderencia))
          ? parsePtNumber(serverRow.aderencia)
          : calculateAdherence(nextRow);
    ...
    const syncPromise = (async () => {
      ...
      try {
        ...
        const nextRows = rows.map((row) => {
          const normalized = {
            ...row,
            cadencia: row.cadencia ?? row.volume ?? 0,
            carregado: row.carregado ?? 0,
            status: normalizeFreteStatus(row.status)
          };

          normalized.aderencia = Number.isFinite(parsePtNumber(row.aderencia))
            ? parsePtNumber(row.aderencia)
            : calculateAdherence(normalized);
        ...
        console.error("[fretes2] erro ao atualizar:", e);
  ...
  window.addEventListener("DOMContentLoaded", () => {
    init().catch((e) => {
      console.error("[fretes2] falha na inicialização:", e);
