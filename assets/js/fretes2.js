  ...
  async function carregarCadastrosFretes() {
    ...
    try {
      ...
      if (possuiDados) {
        ...
        console.warn(
          "[fretes2] cadastros da planilha vazios; mantendo os dados de contingência."
        );
      }
    } catch (e) {
      console.error("[fretes2] erro ao carregar cadastros:", e);
      console.warn(
        "[fretes2] usando os cadastros de contingência existentes no arquivo."
      );
    }
...
<div class="page">
  ...
  function fillModalSelectors() {
    setSelectOptions(
      MODAL.regional(),
      DIRECTORY.regionais.map(upper),
      "SELECIONE A REGIONAL"
    );

    setSelectOptions(
      MODAL.cliente(),
      DIRECTORY.clientes.map(upper),
      "SELECIONE O CLIENTE"
    );
    ...
    if (regionalEl && !regionalEl.dataset.nfCadastrosBound) {
      ...
      regionalEl.addEventListener("change", () => {
        ...
        const filiais =
          (DIRECTORY.filiaisPorRegional?.[reg] || []).map(upper);
    ...
    if (filialEl && !filialEl.dataset.nfCadastrosBound) {
      ...
      filialEl.addEventListener("change", () => {
        ...
        const contatos =
          (DIRECTORY.contatosPorFilial?.[filial] || [])
            .map((c) => upper(c.nome))
            .filter(Boolean);
